...
const ApprovalModal = ({ isOpen, onClose, onApprove, onReject, rule, actionType }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (actionType === 'approve') {
        await onApprove(rule?.id, comments);
      } else {
        await onReject(rule?.id, comments);
      }
      setComments('');
      onClose();
    } catch (error) {
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-semibold text-gray-900">
              {actionType === 'approve' ? 'Approve Rule Set' : 'Reject Rule Set'}
            </h2>
          </div>
          <button
            onClick={handleClose}
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-gray-50 rounded-lg p-4 space-y-2">
            <h3 className="text-sm font-medium text-gray-700">Rule Set Details</h3>
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <span className="text-gray-500">Rule Name:</span>
                <span className="ml-2 font-medium text-gray-900">{rule?.ruleName}</span>
              </div>
              <div>
                <span className="text-gray-500">Regime:</span>
                <span className="ml-2 font-medium text-gray-900">{rule?.regimeType}</span>
              </div>
              <div>
                <span className="text-gray-500">Segment:</span>
                <span className="ml-2 font-medium text-gray-900">{rule?.segmentName}</span>
              </div>
              <div>
                <span className="text-gray-500">Created By:</span>
                <span className="ml-2 font-medium text-gray-900">
                  {rule?.createdByUser?.full_name || rule?.createdBy || 'Unknown'}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className={`flex items-start gap-3 p-4 rounded-lg ${
            ...
            <div className="text-sm">
              ...
              <p className={`mt-1 ${
                ...
                {actionType === 'approve' ? 'This action will approve the rule set and make it active for use in classification.' : 'This action will reject the rule set and send it back to the creator for revision.'}
              </p>
            </div>
          </div>
        </div>
